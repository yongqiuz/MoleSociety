...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hash = searchParams.get('codeHash');
    
    if (address) setUserAddress(address);
    if (hash) setCodeHash(hash);
    
    // 获取出版社统计数据
    const fetchStats = async () => {
      try {
        // 这里你可以调用后端的API，需要传递地址和激活码进行验证
    ...
    <div className="min-h-screen bg-[#0f172a]">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            <div>
              ...
              <p className="text-gray-400">出版社地址: {userAddress}</p>
              <p className="text-sm text-gray-500 mt-1">激活码: {codeHash}</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl p-6 hover:border-cyan-500/30 transition">
            <h3 className="text-xl font-semibold mb-2 text-gray-300">激活码状态</h3>
            <p className="text-3xl font-bold text-yellow-400">有效</p>
        ...
        <div className="mt-8 pt-6 border-t border-gray-700 text-center text-gray-500 text-sm">
          ...
          <p className="mt-1 text-xs">系统版本: 1.2.0</p>
        </div>
      </div>
    </div>
  );
