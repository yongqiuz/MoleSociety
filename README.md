...
- 协议可扩展：当前是 Go + Vue 原型，后续可平滑演进到 ActivityPub、WebSocket、Matrix、libp2p 等体系

本仓库包含：

- 前端：Vue 3 + Vite + TypeScript + Tailwind CSS
- 后端：Go + Gorilla Mux + Redis
- 区块链中继：go-ethereum / Monad RPC 的轻量 Relay 兼容层
- 合约子项目：Foundry + Solidity（保留为链上身份 / 媒体哈希锚定扩展）
- 一键启动脚本：Windows `start-dev.cmd`
...
- 现在的数据库是“内存状态 + Redis 快照”的原型设计，不是生产级分库分表方案
...
当前 Go 后端实现不是传统 ORM + 关系型数据库模式，而是：

- 内存切片持有运行时状态
- Redis 存储快照
- 请求读写通过 `SocialService` 聚合操作完成
...
当前版本的“数据库物理模型”实际是 **Redis 快照模型**。

后端将整个领域集合序列化为 JSON，并写入以下 Key：
...
即使现在仍然在一个 Go 服务中，这种边界也已经在代码结构和实体模型中初步形成。
...
```bash
cd E:\Whale-Vault-NFT_Plus-main
